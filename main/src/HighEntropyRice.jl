...
"""
function encode_value(value::Int, k::Int, divisor::Int, encoded::BitVector)
    quotient = value ÷ divisor
    remainder = value % divisor

    # println("Encoding: ", value)
    append!(encoded, unary_encode(quotient))
    append!(encoded, binary_encode(remainder, k))
end
...
function rice_encode(data::Vector{Int})::BitVector
    ...
    encode_value(data[1], k, rice_param, encoded)

    for i in 2:length(data)
        #Encode the difference between this and the last entry        
        encode_value(data[i]-data[i-1], k, rice_param, encoded)
    end
...
Encodes the given value using Rice compression.
"""
function decode_value(pos::Int, k::Int, divisor::Int, encoded::BitVector)::Tuple{Int, Int}
    quotient, pos = unary_decode(encoded, pos)
    remainder, pos = binary_decode(encoded, pos, k)
...
# Test array
data = Int[100, 101, 102, 103, 104, 200, 205, 208, 300, 306, 500, 502, 510, 1603]
